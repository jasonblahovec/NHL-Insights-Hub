...
def append_onice_info(df_plays, df_forwards, df_defense, df_goalies):
    ...
    df_forwards_key = df_forwards.withColumn("code",f.expr("concat(position,sweaterNumber)"))[['game_id','side','code','playerId',"default_last_name"]]
    df_defense_key = df_defense.withColumn("code",f.expr("concat(position,sweaterNumber)"))[['game_id','side','code','playerId',"default_last_name"]]
    df_goalies_key = df_goalies.withColumn("code",f.expr("concat(position,sweaterNumber)"))[['game_id','side','code','playerId',"default_last_name"]]

    home_joined_df = home_exploded_df.join(
        df_forwards_key.union(df_defense_key.union(df_goalies_key)),
        (home_exploded_df["game_id"] == df_forwards_key["game_id"]) &
        (f.expr("side = home_team")) &
        (f.expr("code = home_onice_code")),
        "left"
    )
    away_joined_df = away_exploded_df.join(
        df_forwards_key.union(df_defense_key.union(df_goalies_key)),
        (away_exploded_df["game_id"] == df_forwards_key["game_id"]) &
        (f.expr("side = away_team")) &
        (f.expr("code = away_onice_code")),
        "left"
    )
    home_pivoted_df = home_joined_df.groupBy([x for x in home_joined_df.columns if x not in df_forwards_key.columns and 'onice_code' not in x]).agg( \
    ...
    away_pivoted_df = away_joined_df.groupBy([x for x in away_joined_df.columns if x not in df_forwards_key.columns and 'onice_code' not in x]).agg( \
    ...
    return home_pivoted_df.join(away_pivoted_df,[x for x in away_joined_df.columns if x not in df_forwards_key.columns and 'onice_code' not in x], "inner")
...
def playbyplaydf(playbyplay_text):
    ...
    df_result = df_result.withColumn("away_onice_array1", f.expr("case when size(home_onice_array) = 0 then slice(on_ice, 1, num_on_ice+1) else away_onice_array end "))
    df_result = df_result.withColumn("home_onice_array1", f.expr(f"case when size(home_onice_array) = 0 then slice(on_ice, num_on_ice + 2, size(on_ice)) else home_onice_array end"))
...
errors = []
for _i,GAMEID in enumerate(range(2022020001,2022020010)):
    try:
...
df_plays_with_onice = append_onice_info(df_plays, df_forwards, df_defense, df_goalies)
df_plays_with_onice.write.format("parquet").save("/FileStore/NHL_PLAYS_DB/plays_with_onice", mode = 'overwrite')

# COMMAND ----------

spark.read.format("parquet").load("/FileStore/NHL_PLAYS_DB/plays_with_onice").display()
