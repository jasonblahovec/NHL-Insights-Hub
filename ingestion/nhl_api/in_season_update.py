...
def run_api_update(year, season_type, last_ingested_game_pk):
    """apply the classes above to the requested year:"""
    games = []
    if season_type in ['01','02']:
        foundLast = False
        gameid = last_ingested_game_pk
        while not foundLast:          
...
def get_last_ingested_game(bucket_name, output_destination, year, game_type):
    rdd = spark.read.format('parquet').load(f"gs://{bucket_name}/{output_destination}/gameinfo") \
        .groupBy("season","game_type") \
            .agg(f.count('game_pk').alias("games")
                    ,f.min('game_pk').alias("first_pk")
                    ,f.max('game_pk').alias("last_pk")) \
            .where(f.expr(f"substr(season,0,4) = '{year}'")) \
            .where(f.expr(f"game_type = '{game_type}'")) \
            .collect()
    return rdd[0]['last_pk']
...
if __name__ == "__main__":
    ...
    parser.add_argument("--year", type=int, help="NHL Season for data retrieval")
    ...
    spark = pyspark.sql.SparkSession.builder \
        .appName("NHL Data Ingestion to GCP - In Season Update") \
        .getOrCreate()
    ...
    last_ingested_game_pk = get_last_ingested_game(bucket_name, output_destination, _year, _game_type)
    print(f"Found last ingested id {last_ingested_game_pk} at location {bucket_name}{output_destination} for {_year} season.")
    games = run_api_update(_year, __game_type, int(last_ingested_game_pk))
